...
void THcPShowerCalib::Init() {
  ...
  gROOT->Reset();
  // char* fname = Form("/lustre/expphy/volatile/hallc/spring17/hdbhatt/group/ROOTfiles/defocused_fall/%s.root",fPrefix.c_str());
...
void THcPShowerCalib::CalcThresholds() {
  ...
  fLoThr = gmean - 3.0*gsigma;
  fHiThr = gmean + 3.0*gsigma;
