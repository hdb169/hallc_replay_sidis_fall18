...
class THcPShowerCalib {
 ...
 private:
  ...
  Double_t faeroMin;              // Threshold noble gas Cerenkov signal, p.e.------------------------------------->nov25
...
void THcPShowerCalib::Init() {
  ...
  fTree->SetBranchAddress("P.cal.etottracknorm", &P_cal_etottracknorm,&b_P_cal_etottracknorm);//---------------------------->>>>
//----------------------------oct 4 P.cal.etottracknorm 
  fTree->SetBranchAddress("P.aero.npeSum", &P_aero_npeSum,&b_P_aero_npeSum);//---------------------------->>>>
...
bool THcPShowerCalib::ReadShRawTrack(THcPShTrack &trk, UInt_t ientry) {
 ...
 if(!good_cal) return 0;//--------------------------------------------------------------------------------oct4
  ...
  bool good_aero = P_aero_npeSum >= faeroMin ; //-----------------------------------------------------------------nov25
  if(!good_aero) return 0;//--------------------------------------------------------------------------------nov25
